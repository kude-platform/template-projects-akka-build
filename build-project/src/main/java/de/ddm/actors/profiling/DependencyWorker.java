...
public class DependencyWorker extends AbstractBehavior<DependencyWorker.Message> {
	...
	private Behavior<Message> handle(TaskMessage message) {
		...
		//sleep for 10 seconds to simulate the time it takes to check the inclusion dependency and log every second
		for (int i = 0; i < 10; i++) {
			try {
				Thread.sleep(1000);
				this.getContext().getLog().info("Demo Sleeping for 10 seconds, slept for " + i + " seconds");
		...
		final LargeMessageProxy.LargeMessage completionMessage =
				new DependencyMiner.CompletionMessage(this.getContext().getSelf(), message, true);
